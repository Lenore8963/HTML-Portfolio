...
import { chinaArticles, chinaRecognitions, chinaSummary } from "@/data/journalism-china";

export const metadata: Metadata = {
  title: "Journalism — China",
  description: chinaSummary,
};

export default function JournalismChinaPage() {
  return (
    <>
      <ReadingProgress />
        ...
        <Container className="pt-20 pb-14">
          <FadeIn className="flex flex-col items-center text-center">
            <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-primary">
              <Newspaper className="h-3.5 w-3.5" /> China
            </span>
            <h1 className="mt-5 text-balance text-4xl font-bold tracking-tight sm:text-6xl">
              Journalism in <span className="text-gradient">China</span>
            </h1>
      ...
      <Container className="py-16">
        ...
        <div className="mt-24">
          <SectionHeading icon={<Globe2 className="h-7 w-7" />} eyebrow="Recognition">
            International Recognition
          </SectionHeading>
          <div className="masonry">
            {chinaRecognitions.map((item) => (
              <RecognitionCard key={item.category + (item.title ?? "")} item={item} />
            ))}
          </div>
        </div>
      </Container>
